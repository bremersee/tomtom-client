...
public class DefaultRoutingClientTest extends Setup {
  ...
  public void testLongRouting() throws Exception {
    ...
    RoutingRequest req = RoutingRequest
        ...
        .departAt(new Date(System.currentTimeMillis() + 1000L * 60L * 60L).toInstant())
        .build();
  ...
  public void testShortRoutingWithPost() throws Exception {
    ...
    RoutingRequest req = RoutingRequest
        ...
        //.alternativeType(AlternativeType.ANY_ROUTE) // cannot specify alternativeType without route to reconstruct
        .instructionsType(InstructionsType.TAGGED)
        .routeRepresentation(RouteRepresentation.POLYLINE)
        .travelMode(TravelMode.PEDESTRIAN)
        .routeType(RouteType.FASTEST)
        .computeBestOrder(Boolean.FALSE) // cannot optimize waypoints and calculate alternative routes at same time!
